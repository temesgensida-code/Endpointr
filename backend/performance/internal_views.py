"""Internal endpoints for Go metrics-service to update PerfTestRun status."""
...
def update_perf_run(request, run_id):
    ...
    from performance.models import PerfTestRun
    try:
        run = PerfTestRun.objects.get(pk=run_id)
    ...
    if new_status:
        ...
        elif new_status in ("completed", "failed", "cancelled") and not run.finished_at:
